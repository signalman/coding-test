...
public class Main18 {
    ...
    static List<Main17.Virus> viruses = new ArrayList<>();
    static Main17.Virus[] active;
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < N; j++) {
                ...
                if (arr[i][j] == 0) {
                    ...
                    viruses.add(new Main17.Virus(i, j, 0));
                }
            }
        }
    ...
    static void spreadVirus(int emptySpace) {
        Queue<Main17.Virus> q = new LinkedList<>();
        boolean[][] infected = new boolean[N][N];

        for (int i = 0; i < M; i++) {
            Main17.Virus virus = active[i];
            infected[virus.x][virus.y] = true;
            q.add(virus);
        }

        while (!q.isEmpty()) {
            Main17.Virus virus = q.poll();
            ...
            for (int i = 0; i < 4; i++) {
                ...
                infected[nx][ny] = true;
                q.add(new Main17.Virus(nx, ny, virus.time + 1));
            }
        }
    }
}
