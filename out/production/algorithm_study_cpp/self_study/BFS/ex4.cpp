...
queue<pair<int, int>> Q2; //지훈이에 대한 queue
...
int main(){
  ...
  for(int i=0; i<n; i++){
    for(int j=0; j<m; j++){
      if(board[i][j]=='F')  { dist1[i][j]=0; Q1.push({i,j});}
      if(board[i][j]=='J')  { dist2[i][j]=0; Q2.push({i,j});}
    }
  }
  //불에 대한 BFS
  while(!Q1.empty()){
  ...
  //지훈이에 대한 BFS
        ...
        for(int dir=0; dir<4; dir++){
          int nx = cur.X + dx[dir];
          int ny = cur.Y + dy[dir];
          if(nx<0 ||nx>=n || ny<0 || ny>=m) {cout<<dist2[cur.X][cur.Y]+1; return 0;}
